...
from video2mesh.options import BackgroundMeshOptions, COLMAPOptions, MaskDilationOptions, MeshDecimationOptions, MeshFilteringOptions, MeshReconstructionMethod, PipelineOptions, StorageOptions, WebXROptions
...
class Interface: 
    ...
    def getInterface() -> gr.Blocks:
        def start_pipeline(o):
            options=PipelineOptions(num_frames=int(o[num_frames]), frame_step=int(o[frame_step]), estimate_pose=o[estimate_pose], estimate_depth=o[estimate_depth], background_only=o[background_only], align_scene=o[align_scene], log_file=o[log_file], use_inpainting=int(o[use_inpainting]))
            storage_options=StorageOptions(dataset_path=o[dataset_path], output_path=o[output_path], overwrite_ok=o[overwrite_ok], no_cache=o[no_cache])
            decimation_options=MeshDecimationOptions(num_vertices_background=int(o[num_vertices_background]), num_vertices_object=int(o[num_vertices_object]), max_error=o[decimation_max_error])
            dilation_options=MaskDilationOptions(num_iterations=int(o[dilate_mask_iter]))
            filtering_options=MeshFilteringOptions(max_pixel_distance=int(o[max_pixel_dist]), max_depth_distance=o[max_depth_dist], min_num_components=int(o[min_num_components]))
            colmap_options=COLMAPOptions(is_single_camera=o[is_single_camera], dense=o[dense], quality=o[quality], binary_path=o[binary_path], vocab_path=o[vocab_path])
            static_mesh_options=BackgroundMeshOptions(reconstruction_method=MeshReconstructionMethod.from_string(o[mesh_reconstruction_method]), depth_mask_dilation_iterations=int(o[depth_mask_dilation_iterations]), sdf_volume_size=float(o[sdf_volume_size]), sdf_voxel_size=float(o[sdf_voxel_size]), sdf_max_voxels=int(o[sdf_max_voxels]))
            webxr_options=WebXROptions(webxr_path=o[webxr_path], webxr_url=o[webxr_url], webxr_add_ground_plane=o[webxr_add_ground_plane], webxr_add_sky_box=o[webxr_add_sky_box])
            
            pipeline = Pipeline(options=options, storage_options=storage_options, decimation_options=decimation_options, dilation_options=dilation_options, filtering_options=filtering_options, colmap_options=colmap_options, static_mesh_options=static_mesh_options, webxr_options=webxr_options)
            pipeline.run() 
        ...
        with gr.Blocks(theme=gr.themes.Default()) as demo:
            ...
            with gr.Accordion("Pipeline Options", open=True):
                with gr.Row():
                    ...
                    with gr.Column():
                        use_inpainting = gr.Dropdown(choices=["0","1","2","3","4"], value="0", multiselect=False,label="use_inpainting", interactive=True)
            ...
            with gr.Accordion("WebXROptions", open=False):
                with gr.Row():
                    ...
                    with gr.Column():
                        webxr_add_ground_plane = gr.Checkbox(value=False, label="webxr_add_ground_plane", interactive=True)
                ...
                with gr.Row():
                    with gr.Column():
                        mesh_reconstruction_method = gr.Dropdown(choices=[method.name.lower() for method in
                                    BackgroundMeshOptions.supported_reconstruction_methods], value='tsdf_fusion', label="mesh_reconstruction_method", interactive=True)
                    with gr.Column():
                        depth_mask_dilation_iterations = gr.Number(value=32, label="depth_mask_dilation_iterations", interactive=True)
                    with gr.Column():
            ...
            with gr.Accordion("Mesh Decimation Options", open=False):
                with gr.Row():
                    with gr.Column():
                        num_vertices_background = gr.Number(value=2 ** 14, label="num_vertices_background", interactive=True)
                    with gr.Column():
            ...
            with gr.Accordion("COLMAP Options", open=False):
                with gr.Row():
                    ...
                    with gr.Column():
                        quality = gr.Dropdown(choices=Interface.quality_choices, value="low", multiselect=False, label="quality", interactive=True)
                    with gr.Column():
                        binary_path = gr.Text(value="/usr/local/bin/colmap", label="binary_path", interactive=True)
                    with gr.Column():
                        vocab_path = gr.Text(value="/root/.cache/colmap/vocab.bin", label="vocab_path", interactive=True)

            btn = gr.Button(value="Start Pipeline")
            inputs = {num_frames, frame_step, estimate_pose, estimate_depth, background_only, align_scene, log_file, use_inpainting, 
                      ...
                      mesh_reconstruction_method, depth_mask_dilation_iterations, sdf_volume_size, sdf_voxel_size, sdf_max_voxels,
