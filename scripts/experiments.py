...
class CMUPanopticDataset:
    ...
    calibration_filename_formatter = 'calibration_{:d}.json'.format
    kinect_calibration_filename_formatter = 'kcalibration_{:d}.json'.format

    sync_tables_filename_formatter = 'synctables_{:d}.json'.format
    kinect_sync_tables_filename_formatter = 'ksynctables_{:d}.json'.format
    ...
    def load_sync_table(self):
        ...
        with open(path, 'r') as f:
            sync_table_data = json.load(f)

        return SyncTable(sync_table_data)
    ...
    def load_kinect_sync_table(self):
        ...
        with open(path, 'r') as f:
            kinect_sync_table_data = json.load(f)
    ...
    def get_synced_frame_data(self, frame_index: int, kinect_node: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        color_index = np.argmin(universal_time - color_sync_table.universal_time - 6.25)
        ...
        depth_index = np.argmin(universal_time - depth_sync_table.universal_time)
